    ...
    if query.data == 'add_habit':
        ...
        if user_id not in user_habits or not user_habits[user_id]:
            ...
            for habit in user_habits[user_id]:
                ...
                habits_text += f"• {habit['name']} (напоминание: {time_str})\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('state') == 'adding_habit':
        ...
        context.user_data['current_habit'] = len(user_habits[user_id]) - 1
        
        await update.message.reply_text(
            f"Привычка '{habit_name}' добавлена!\n\n"
            "Теперь отправь мне время для напоминания в формате ЧЧ:ММ\n"
            "Например: 09:00"
        )
    
    elif context.user_data.get('state') == 'setting_time':
        try:
